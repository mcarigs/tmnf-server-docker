...
/*
*******************************************************************************
***** FufiWidgets by oorf | fuckfish (ASECO 2.2.x+ & XASECO 1.04 Version) *****
*******************************************************************************
**************************** http://fish.stabb.de *****************************
*******************************************************************************
...
* working properly.
* 1. Unrar the archive into the (X)ASECO folder
...
* bronze time and a TMX link (if you installed oliverde8's TMX-Info plugin
...
class FufiWidgets extends Plugin {
	...
	function loadSettings(){
		...
		$dedimaniaSettings["enabled"] = $this->stringToBool(strval($xml->dedimaniawidget->enabled));
	...
	function showKarmaWidget($login=''){
		...
		if (IN_XASECO){
			...
			if (isset($challenge->data['DB_ID'])){
				...
				$query = 'Select Id from challenges where Uid="'.$challenge->uid.'"';
	...
	function getCTUEntry($rec, $i, $highlite, $entry, $showPoints, $width=0, $topCount){
		...
		$entry_ = str_replace(array("%x_rank%", "%y%", "%rank%", "%x_score%", "%score%", "%x_nick%", "%nick%"),	array(2.5, -2*$i-1.2, $rank.'.', 6.8, $score, 7.2, htmlspecialchars($this->getValidUTF8String($nick))), $entry);
		return $entry_;
	...
	function unicodetoutf8($string) {
		// Latin 1
		$string = str_replace("\xFF\xFE", '', $string); // Unicodeeintrag entfernen
		$string = str_replace("\x0D\x00\x0A\x00", '<br>', $string); // konvertiert \r\n zu <br>
	...
	function showChallengeInfo($login, $state = 0){
		...
		//little workaround that displays a tmx button if the tmxinfo plugin is available

		if (IN_XASECO){
			$tmxInfo = true;
		} else {
			$tmxInfo = $this->Aseco->getPlugin('TMX_info');
		}
		...
		if ($tmxInfo && ($state == 2)){
			...
			if (IN_XASECO){
				chat_tmxinfo($this->Aseco, $command);
			} else {
				$tmxInfo->TMX_info_manialink($command);
			}
			return;
		}
		if (!$tmxInfo && $state==1){
		...
		$page = str_replace(array("%manialinksid%", "%widgetid%", "%widgetpos%", "%trackname%", "%bronzetime%", "%silvertime%", "%goldtime%", "%authortime%", "%author%"),
		array($this->manialinksID,	$this->challengeWidgetID, $this->settings["challengewidget"]["states"][$state]["pos"], htmlspecialchars($this->getValidUTF8String($challenge->name)), formatTime($challenge->bronzetime), formatTime($challenge->silvertime), formatTime($challenge->goldtime), formatTime($challenge->authortime), htmlspecialchars($this->getValidUTF8String($challenge->author))), $page);
		if ($login){
			$this->sendMLtoLogin($login, $page, 'Sending challenge widget', 'cw');
		} else {
			$this->sendML($page, 'Sending challenge widget', 'cw');
		}
	}
	...
	function getLiveRanksList(){
		...
		if ($this->Aseco->client->isError()){
			$this->Aseco->console("FufiWidgets: LiveRankingsError, reverting to last valid ranking");
	...
	function hideRaceWidgets(){
		...
		if (!$this->settings['karmawidget']['states'][6]['enabled']){
	...
	function doNewChallenge(){
		if ($this->Aseco->debug) $this->Aseco->console('NewChallenge');
	...
	function executeNewChallenge($resetUIHashs = true){
		...
		if (IN_XASECO){
			$this->Aseco->client->query('GetCurrentChallengeInfo', array());
			$challenge = $this->Aseco->client->getResponse();
			$this->xasecoChallenge = new Challenge($challenge);
	 ...
	 */
	function doEndRound(){
		if ($this->Aseco->server->gameinfo->mode == 0 || $this->Aseco->server->gameinfo->mode == 0){
			$this->showRecordsWidgets();
		}
	}
...
if (IN_XASECO){
	...
	$fufiWidgets->setVersion($fufiWidgetsVersion);
	...
	if (in_array('DediRecs', $fufiWidgets->settings["sblwidget"]["used"]) || $fufiWidgets->settings["dedimaniawidget"]["enabled"]){
	...
	Aseco::registerEvent('onNewChallenge', 'fufiwidgets_newChallenge');
	...
	Aseco::registerEvent('onEndRace', 'fufiwidgets_endRace');
	...
	$_PLUGIN->setVersion($fufiWidgetsVersion);
	...
	$_PLUGIN->addEvent('onEndRace', 'doEndRace');

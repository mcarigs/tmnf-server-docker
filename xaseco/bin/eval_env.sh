...
# Config
SERVER_NAME=${SERVER_NAME:-Trackmania Server} && \
    config+=( "SERVER_NAME" )

SERVER_NATION=${SERVER_NATION:-USA} && \
    config+=( "SERVER_NATION" )
...
MASTERADMIN_IP=${MASTERADMIN_IP} && \
    config+=( "MASTERADMIN_IP" )
...
MUSIC_SERVER=${MUSIC_SERVER:?WARN | MUSIC_SERVER is not defined. Custom music will not be enabled} && \
    config+=( "MUSIC_SERVER" )
echo "INFO | MUSIC_SERVER: ${MUSIC_SERVER}"
